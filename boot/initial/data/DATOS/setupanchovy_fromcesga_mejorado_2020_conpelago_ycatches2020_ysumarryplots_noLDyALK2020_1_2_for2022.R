...
# setwd("~/GADGET/Assessment/Assessment_2022")
library(mfdb)
library(gdata)
...
gd <- gadget_directory("Anchovy2022_def_estesi")
...
#pelago = read.xls("./DATOS/pelago_99_14_simp.xls")#estas son las portuguesas
#ecocadiz = read.xls("./DATOS/ecocadiz_04_13_simp.xls")
#el 2013 debe coincidir con el total de ecocadiz0813 estas son las espa??olas
sar = read.xls("./DATOS/sar_98_07.xls")
catch_age=read.xls("./DATOS/catch_at_age_numbers88_13.xls")
length_age=read.xls("./DATOS/catch_length_at_age_88_13_3.xls")
#weight_age=read.csv("./DATOS/catches_weight_at_age_kg88_13_3.csv",strip.white = TRUE, sep=",", na.strings= c("999", "NA", " ", ""),stringsAsFactors=FALSE)  
#weight_age=read.csv("./DATOS/catches_weight_at_age_kg88_13_2.csv")
weight_age=read.xls("./DATOS/catches_weight_at_age_kg88_13_5.xls")
...
Catches_Algarve<-add_row(Catches_Algarve, 
                         YEAR=2017, 
                         SEASON=1:4, 
                         totalton=c(10079.6,1953.5,12871,1206.6)*0.001) #porque estaba en kg se multiplica por 0.001 para que quede en tons CREO 2019 hay que verificar
...
Catches_Algarve<-add_row(Catches_Algarve, 
                         YEAR=2018, 
                         SEASON=1:4, 
                         totalton=c(1431.3,11785.3,52035.7,83.6)*0.001)  #porque estaba en kg se multiplica por 0.001 para que quede en tons CREO 2019 hay que verificar
Catches_Algarve<-add_row(Catches_Algarve, 
                         YEAR=2019, 
                         SEASON=1:4, 
                         totalton=c(0,0,4,109))  #porque estaba en ya en tons ver ./DATOS/Algarve/Landings_Metier_9aS_alg_2019.xlsx
Catches_Algarve<-add_row(Catches_Algarve, 
                         YEAR=2020, 
                         SEASON=1:4, 
                         totalton=c(1.6059,67.2002,	73.5084,12.23))
#estaba en Kg pero en el excel lo he multiplicado por 0.001
#sacado de ./Assessment/Assessment_2022/Datos_algarve_capturas
#Las del 2021 sacada de DATOS/Algarve/Tabla_capturas_ANE_2021_algarve.xlsx
Catches_Algarve<-add_row(Catches_Algarve, 
                         YEAR=2021, 
                         SEASON=1:4, 
...
Year4bio2num<-2021
NUMPT<-data.frame(YEAR=rep(1989:Year4bio2num,each=4), 
                  SEASON=rep(1:4,each=length(1989:Year4bio2num)),
                  NUMBERPT=rep(0,each=length(1989:Year4bio2num)*4),
                  NUMBERSP=rep(0,each=length(1989:Year4bio2num)*4))
BIOMTOT<-data.frame(YEAR=rep(1989:Year4bio2num,each=4), 
                    SEASON=rep(1:4,each=length(1989:Year4bio2num)),
                    TONPT=rep(0,each=length(1989:Year4bio2num)*4),
                    TONSP=rep(0,each=length(1989:Year4bio2num)*4))
a<-1
for (i in 1989:2013){
  for (j in 1:4){
  bioq<-readWorksheetFromFile(paste("./DATOS/Taledas_allfleets_1988_2016/BOQUERON_ALK_",i,".xls",sep=""), 
                              sheet = paste(j,"Q",sep=""), 
                              header = F, 
                              startCol = 9, 
                              startRow = 2, 
                              endCol = 9, 
                              endRow = 2, 
                              useCachedValues=T)*0.001 #Para que quede en toneladas (está en kilos)
  
numberq<-readWorksheetFromFile(paste("./DATOS/Taledas_allfleets_1988_2016/BOQUERON_ALK_",i,".xls",sep=""), 
                               sheet = paste(j,"Q",sep=""), 
                               header = F, 
                               startCol = 9, 
                               startRow = 38, 
...
for (i in 2014:Year4bio2num){
  for (j in 1:4){
    bioq<-readWorksheetFromFile(paste("./DATOS/Taledas_allfleets_1988_2016/BOQUERON_ALK_",i,".xls",sep=""), 
                                ...
                                endCol = 9, 
                                endRow = 2, 
                                useCachedValues=T)*0.001 #Para que quede en toneladas
...
for (i in c(1999,2001:2003,2005:2009)){
  ...
  a<-readWorksheetFromFile("./DATOS/Acústica en 9aS_TallasyEdades.xls", 
                           sheet = paste(i,"Port",sep=""), 
                           header = TRUE, 
                           startCol = 13, 
                           startRow = 5, 
                           endCol = 17, 
...
for (i in c(2010,2013,2014)){
  ...
  a<-readWorksheetFromFile("./DATOS/Acústica en 9aS_TallasyEdades.xls", 
                           ...
                           endRow = 35, 
                           useCachedValues=TRUE)
  b<-readWorksheetFromFile("./DATOS/Acústica en 9aS_TallasyEdades.xls", 
                           sheet = paste(i,"Port",sep=""), 
                           header = F, 
                           startCol = 9, 
                           startRow = 2, 
...
for (i in c(2015:2022)){
  name<-paste("Pelago",i,sep="")
  a<-readWorksheetFromFile(paste("./DATOS/PELAGO/Pelago",i,".xls",sep=""),
                           sheet = "Sheet1", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 6, 
  ...
  b<-readWorksheetFromFile(paste("./DATOS/PELAGO/Pelago",i,".xls",sep=""), 
                           sheet = "Sheet1", 
                           header = F, 
                           startCol = 10, 
                           startRow = 5, 
...
mybiglist$Pelago2003$year <-2002
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/ANE_PELAGO15_Results.xls", 
                           sheet = "9a S", 
                           header = TRUE, 
                           startCol = 13, 
                           startRow = 2, 
 ...
 # b<-readWorksheetFromFile(paste("./DATOS/PELAGO/Pelago",i,".xls",sep=""), sheet = "Sheet1", 
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/PELAGO16_ANE_RESULTS_FINAL.xls", 
                           sheet = "AbundanceBiomassANE_FINAL", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 66, 
                           endCol = 2, 
...
i<-2016
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/results-ane-pel17_fv.xlsx", 
                           sheet = "CAD", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 49, 
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/results-ANE-PEL18.xlsx", 
                           sheet = "algarve+cadiz", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 1, 
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/results-ANE-PEL19.xlsx", 
                           sheet = "algarve+cadiz", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 1, 
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/results-ANE-PEL20.xlsx", 
                           sheet = "algarve+cadiz", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 1, 
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/results-ANE-PEL21.xlsx", 
                           sheet = "algarve+cadiz", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 1, 
  ...
  a<-readWorksheetFromFile("./DATOS/PELAGO/results-ANE-PEL22.xlsx", 
                           sheet = "algarve+cadiz", 
                           header = TRUE, 
                           startCol = 1, 
                           startRow = 1, 
 ...
 #  d<-readWorksheetFromFile("./DATOS/PELAGO/biq2000.xls", sheet = "ALgarve", 
 #                           header = TRUE, startCol = 1, 
 #                           startRow = 1, endCol = 2, 
 #                           endRow = 30, useCachedValues=TRUE)
 #  c<-readWorksheetFromFile("./DATOS/PELAGO/biq2000.xls", sheet = "CADIZ", 
 #                              header = TRUE, startCol = 1, 
 #                              startRow = 1, endCol = 2, 
 #                              endRow =30, useCachedValues=TRUE)
 #  d[is.na(d)] <- 0
...
name<-"alPelago2014" 
d<-readWorksheetFromFile("./DATOS/PELAGO/PELAGO14_ANE_ALK_WCOAST_sg.xlsx", 
                         sheet = "alg", 
                         header = TRUE, 
                         startCol = 6, 
                         startRow = 7, 
...
c<-readWorksheetFromFile("./DATOS/PELAGO/PELAGO14_ANE_ALK_WCOAST_sg.xlsx", 
                         sheet = "cad", 
                         header = TRUE, 
                         startCol = 6, 
                         startRow = 7, 
...
name<-"alPelago2015" 
d<-readWorksheetFromFile("./DATOS/PELAGO/ANE_PELAGO15_Results_sg.xlsx", 
                         sheet = "Algarve", 
                         header = TRUE, 
                         startCol = 17, 
                         startRow = 4, 
...
c<-readWorksheetFromFile("./DATOS/PELAGO/ANE_PELAGO15_Results_sg.xlsx", 
                         sheet = "CADIZ", 
                         header = TRUE, 
                         startCol = 17, 
                         startRow = 4, 
...
name<-"alPelago2016" 
a<-readWorksheetFromFile("./DATOS/PELAGO/PELAGO16_ANE_RESULTS_FINAL_sg.xlsx", 
                         sheet = "ALKs_FINAL", 
                         header = TRUE, 
                         startCol = 8, 
                         startRow = 6, 
...
for (i in c(2004,2006,2007,2009,2010)){
  ...
  bioage0<-readWorksheetFromFile("./DATOS/Acústica en 9aS_TallasyEdades.xls", 
                                 sheet = paste(i,"Esp",sep=""), 
                                 header = F, 
                                 startCol = 5, 
                                 startRow = 92, 
...
for (i in c(2015:2020)){
  ...
  a<-readWorksheetFromFile(paste("./DATOS/ECOCADIZ/ALK_ECOCADIZ_",i,".xls",sep=""), 
                           sheet = "ALK_GENERAL_BOQUERON", 
                           header = TRUE, 
                           startCol = 11, 
                           startRow = 7, 
                           endCol = 15, 
  ...
  b<-readWorksheetFromFile(paste("./DATOS/ECOCADIZ/ALK_ECOCADIZ_",i,".xls",sep=""), 
                           sheet = "ALK_GENERAL_BOQUERON", 
                           header = F, 
                           startCol = 9, 
                           startRow = 3, 
  ...
  bioage0<-readWorksheetFromFile(paste("./DATOS/ECOCADIZ/ALK_ECOCADIZ_",i,".xls",sep=""), 
                                 sheet = "ALK_GENERAL_BOQUERON", 
                                 header = F, 
                                 startCol = 5, 
                                 startRow = 103, 
...
pelago_bio<-readWorksheetFromFile("./DATOS/Acústica en 9aS_TallasyEdades.xls", 
                                  sheet = "BiomasaPort", 
                                  header = F, 
                                  startCol = 3, 
                                  startRow = 35, 
...
# pelbiom2019<-readWorksheetFromFile("./DATOS/PELAGO/results-ANE-PEL19.xlsx", sheet = "algarve+cadiz", 
#                                    header = F, startCol = 3, 
#                                    startRow = 44, endCol = 3, 
#                                    endRow = 44, useCachedValues=TRUE)
...
ecocadiz_bio<-readWorksheetFromFile("./DATOS/Acústica en 9aS_TallasyEdades.xls", 
                                    sheet = "BiomasaEsp", 
                                    header = F, 
                                    startCol = 3, 
                                    startRow = 35, 
...
ecoca_age0<-left_join(ecocadiz_biom,B0)%>%
...
# ecocadiz0813<-read.xls("./DATOS/ecocadiz_tableincolumn.xls")
...
fleet.seinepre<-plyr::ddply(catch_at_length,~year,summarise,
                            Total_catches1=sum(Q1),
                            Total_catches2=sum(Q2), 
                            Total_catches3=sum(Q3), 
                            Total_catches4=sum(Q4))  #suma sobre la longitud
...
# #catches20181cakg<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2018.xlsx", sheet="Hoja2",
#                                         header = F, startCol = 7, 
#                                         startRow = 19, endCol = 7, 
#                                         endRow = 19, useCachedValues=TRUE)
# #catches20182cakgpre<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2018.xlsx", sheet="Hoja2",
#                                         header = F, startCol = 7, 
#                                         startRow = 22, endCol = 7, 
#                                         endRow = 22, useCachedValues=TRUE)
# catches20181alton<-readWorksheetFromFile("./DATOS/Algarve/PNAB_biqueirao_2018.xlsx", sheet="Sheet1",
#                                          header = F, startCol = 5, 
#                                          startRow = 7, endCol = 5, 
#                                          endRow = 7, useCachedValues=TRUE)
# 
# catches20182altonpre<-readWorksheetFromFile("./DATOS/Algarve/PNAB_biqueirao_vendas-diarias_2018.xls", sheet="Sheet1",
...
# catches20191cakg<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2019.xlsx", sheet="Hoja2",
#                                         header = F, startCol = 7, 
#                                         startRow = 19, endCol = 7, 
#                                         endRow = 19, useCachedValues=TRUE)
# catches20192cakgpre<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2019.xlsx", sheet="Hoja2",
#                                            header = F, startCol = 7, 
#                                            startRow = 22, endCol = 7, 
#                                            endRow = 22, useCachedValues=TRUE)
...
# catches20201cakg<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2020.xlsx", sheet="landings2020",
#                                          header = F, startCol = 10,
#                                          startRow = 18, endCol = 10,
#                                          endRow = 18, useCachedValues=TRUE)
# catches20202cakgpre<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2020.xlsx", sheet="landings2020",
#                                          header = F, startCol = 10,
#                                          startRow = 19, endCol = 10,
#                                          endRow = 19, useCachedValues=TRUE)
#  catches20201alton<-1.6#readWorksheetFromFile("./DATOS/Algarve/PNAB_biqueirao_2018.xlsx", sheet="Sheet1",
#                                           #header = F, startCol = 5,
#                                          # startRow = 7, endCol = 5,
#                                           #endRow = 7, useCachedValues=TRUE)
# 
#  catches20202altonpre<-0.2#readWorksheetFromFile("./DATOS/Algarve/PNAB_biqueirao_vendas-diarias_2018.xls", sheet="Sheet1",
#                                          # header = F, startCol = 13,
#                                          # startRow = 14, endCol = 13,
#                                          # endRow = 14, useCachedValues=TRUE)
catches20221cakg<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2022.xls", 
                                        sheet="Sheet1",
                                        header = F, 
                                        startCol = 7,
                                        startRow = 20, 
                                        ...
                                        endRow = 20, 
                                        useCachedValues=TRUE)
catches20222cakgpre<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Ane279a_9aS_Prov Landings Jan-June 2022.xls", 
                                           sheet="Sheet1",
                                           header = F, 
                                           startCol = 8,
                                           startRow = 20, 
...
#el 0.36 es definitivo para 2022
catches20222cakgpre<-catches20222cakgpre+0.39*(catches20221cakg+catches20222cakgpre)#sacado de historico-capturas
catches20221alton<-0
#No hay capturas Algarve 2022
#ver ANE_biqueirão_01012021-30042021 en DATOS/Algarve

#readWorksheetFromFile("./DATOS/Algarve/PNAB_biqueirao_2018.xlsx", sheet="Sheet1",
#header = F, startCol = 5,
# startRow = 7, endCol = 5,
#endRow = 7, useCachedValues=TRUE)

catches20222altonpre<-0#readWorksheetFromFile("./DATOS/Algarve/PNAB_biqueirao_vendas-diarias_2018.xls", sheet="Sheet1",
# header = F, startCol = 13,
# startRow = 14, endCol = 13,
# endRow = 14, useCachedValues=TRUE)
...
BIOMTOT$ALL<-BIOMTOT$TONPT+BIOMTOT$TONSP
 BIOMTOT<-add_row(BIOMTOT, 
                  YEAR=2022, 
                  SEASON=1:2,
                  TONPT=c(catches20221alton,catches20222altonpre),
                  TONSP=c(catches20221cakg[1,1]*0.001,catches20222cakgpre[1,1]*0.001),
                  ALL=c(catches20221alton+catches20221cakg[1,1]*0.001,catches20222altonpre+catches20222cakgpre[1,1]*0.001)) #sacado de Ane279a_9aS_Prov Landings Jan-June 2019.xls porque para Portugal las capturas son cero
 
 BIOMTOTnewyear<-BIOMTOT %>% 
   mutate(newyear=c(1988,1988,rep(1989:2021,each=4))) %>%
   group_by(newyear)%>%
   summarise(catonsur=sum(ALL)) #cuidado aquí el último año es el año anterior al assessment porque va en management calendar
   
   
 discardsbyquarter<-readWorksheetFromFile("./DATOS/Taledas_allfleets_1988_2016/Landings and Discards_9aN and 9a S_2014 on.xlsx", 
                                          sheet="South_9aS",
                                          header = T, 
                                          startCol = 9, 
                                          startRow = 4, 
 ...
 DISCARDS<-add_row(DISCARDS, newyear=2021, newyeardiscardsouth=64, year=2021)
 #sacado de Landings and Discards_9aN and 9a S_2014 on_2021.xlsx #corregidas también las del 2020
 
 ######EStán en management year me las ha pasado Ferni

#save(BIOMTOTnewyear,DISCARDS,file="./Assessment/Assessment2020_pandemia/BIOMTOTnewyeardiscards.Rdata")
...
fleet.seine<-rbind(fleet.seinepre[c(1,2)],
                   fleet.seinepre[c(1,3)],
                   fleet.seinepre[c(1,4)],
                   fleet.seinepre[c(1,5)])
...
for (i in c(1993,1997:2016)){
  ...
  a<-readWorksheetFromFile(paste("./DATOS/ARSA/ARSA_NOV_ALK_",i,".xlsx",sep=""),
                           sheet = "4Q", 
                           header = TRUE, 
                           startCol = 11, 
                           startRow = 5, 
...
arsa_bio<-readWorksheetFromFile("./DATOS/ARSA/WKPELA2018_ane.27.9a_ES_SURVEY_ARSA-Novseries.xlsx", 
                                sheet = "Absolute Indices' estimates", 
                                header = F, 
                                startCol = 15, 
                                startRow = 2, 
                                endCol = 15, 
...
bocadeva_bio<-readWorksheetFromFile("./DATOS/DEPM/DEPM_SSB_estimates.xlsx", 
                                    sheet="Sheet1",
                                header = F, 
                                startCol = 1, 
                                startRow = 2, 
...
for (i in 1989:2013){
  for (j in 1:4){
    ...
    a<-readWorksheetFromFile(paste("./DATOS/Taledas_allfleets_1988_2016/BOQUERON_ALK_",i,".xls",sep=""), 
                             sheet = paste(j,"Q",sep=""), 
                             header = T, 
                             startCol = 1, 
                                startRow = 5, 
                             endCol = 5, 
                                endRow = 37, 
                             useCachedValues=T)
    ldist_cat<- readWorksheetFromFile(paste("./DATOS/Taledas_allfleets_1988_2016/BOQUERON_ALK_",i,".xls",sep=""), 
                                      sheet = paste(j,"Q",sep=""), 
                                      header = T, 
                                      startCol = 8, 
                                      startRow = 5, 
...
catches.agedist<-rbind(catchespre[c(1,2,3)],
                       catchespre[c(1,2,4)],
                       catchespre[c(1,2,5)],
                       catchespre[c(1,2,6)])
...
length.at.age<-rbind(length_agepre[c(1,2,3)],
                     length_agepre[c(1,2,4)],
                     length_agepre[c(1,2,5)],
                     length_agepre[c(1,2,6)])
...
weight.at.age<-rbind(weight_agepre[c(1,2,3)],
                     weight_agepre[c(1,2,4)],
                     weight_agepre[c(1,2,5)],
                     weight_agepre[c(1,2,6)])
...
#IXaGoC
seine_bio<-readWorksheetFromFile("./DATOS/ane 27.9a_Catches by Subdivision.xlsx", 
                                 sheet = "Quarterly Catches", 
                                 header = F, 
                                 startCol = 30, 
                                 startRow = 7, 
                                 endCol = 34, 
...
port_bio<-readWorksheetFromFile("./DATOS/ane 27.9a_Catches by Subdivision.xlsx", 
                                sheet = "Quarterly Catches", 
                                header = F, 
                                startCol = 23, 
                                startRow = 10, 
...
for (i in c(2012,2014:2016)){
  ...
  a<-readWorksheetFromFile(paste("./DATOS/ECOCADIZ-RECLUTAS/ALK_ECO-R_",i,".xls",sep=""), 
                           sheet = "ALK_GENERAL_BOQUERON", 
                           header = TRUE, 
                           startCol = 11, 
                           startRow = 7, 
                           ...
                           endRow = 36, 
                           useCachedValues=TRUE)
  b<-readWorksheetFromFile(paste("./DATOS/ECOCADIZ-RECLUTAS/ALK_ECO-R_",i,".xls",sep=""), 
                           sheet = "ALK_GENERAL_BOQUERON", 
                           header = F, 
                           startCol = 9, 
                           startRow = 3, 
